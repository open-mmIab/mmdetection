...
class DistEvalHook(Hook):
    def __init__(self, dataset, interval=1):
        if isinstance(dataset, Dataset):
            ...
            self.dataset = obj_from_dict(
                dataset, datasets, {"test_mode": True}
            )
        else:
            raise TypeError(
    ...
    def after_train_epoch(self, runner):
        ...
        for idx in range(
            runner.rank, len(self.dataset), runner.world_size
        ):
        ...
        if runner.rank == 0:
            ...
            for i in range(1, runner.world_size):
                tmp_file = osp.join(
                    runner.work_dir, "temp_{}.pkl".format(i)
                )
...
class DistEvalmAPHook(DistEvalHook):
    def evaluate(self, runner, results):
        ...
        if (
            hasattr(self.dataset, "year")
            and self.dataset.year == 2007
        ):
            ds_name = "voc07"
        else:
