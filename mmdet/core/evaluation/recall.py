...
def _recalls(all_ious, proposal_nums, thrs):
    ...
    _ious = np.zeros(
        (proposal_nums.size, total_gt_num), dtype=np.float32
    )
    for k, proposal_num in enumerate(proposal_nums):
        ...
        for i in range(img_num):
            ...
            for j in range(ious.shape[0]):
                ...
                max_ious = ious[
                    np.arange(0, ious.shape[0]), gt_max_overlaps
                ]
    ...
    for i, thr in enumerate(thrs):
        recalls[:, i] = (_ious >= thr).sum(axis=1) / float(
            total_gt_num
        )
...
def eval_recalls(
    ...
    proposal_nums, iou_thrs = set_recall_param(
