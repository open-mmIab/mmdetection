    ...
    if gt_bboxes.shape[0] == 0:
        if area_ranges == [(None, None)]:
            ...
            for i, (min_area, max_area) in enumerate(area_ranges):
                fp[
                    i,
                    (det_areas >= min_area) & (det_areas < max_area),
                ] = 1
        return tp, fp
...
def tpfp_default(
    det_bboxes, gt_bboxes, gt_ignore, iou_thr, area_ranges=None
):
    """Check if detected bboxes are true positive or false positive.
...
def get_cls_results(
    det_results, gt_bboxes, gt_labels, gt_ignore, class_id
):
    """Get det results and gt information of a certain class."""
    cls_dets = [
        det[class_id] for det in det_results
    ]  # det bboxes of this class
    ...
    for j in range(len(gt_bboxes)):
        ...
        cls_gt = (
            gt_bbox[cls_inds, :] if gt_bbox.shape[0] > 0 else gt_bbox
        )
        cls_gts.append(cls_gt)
        if gt_ignore is None:
            cls_gt_ignore.append(
                np.zeros(cls_gt.shape[0], dtype=np.int32)
            )
        else:
...
def eval_map(
    ...
    gt_labels = [
        label if label.ndim == 1 else label[:, 0]
    ...
    for i in range(num_classes):
        ...
        for j, bbox in enumerate(cls_gts):
            if area_ranges is None:
                ...
                for k, (min_area, max_area) in enumerate(
                    area_ranges
                ):
                    num_gts[k] += np.sum(
                        np.logical_not(cls_gt_ignore[j])
    ...
    if scale_ranges is not None:
        ...
        all_ap = np.vstack(
            [cls_result["ap"] for cls_result in eval_results]
        )
...
def print_map_summary(mean_ap, results, dataset=None):
    ...
    precisions = np.zeros(
        (num_scales, num_classes), dtype=np.float32
    )
