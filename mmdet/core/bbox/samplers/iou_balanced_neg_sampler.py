        ...
        if len(neg_inds) <= num_expected:
            ...
            easy_set = set(
                np.where(
                    np.logical_and(
                        max_overlaps >= 0,
                        max_overlaps < self.hard_thr,
                    )
                )[0]
            )
            hard_set = set(
                np.where(max_overlaps >= self.hard_thr)[0]
            )
            easy_neg_inds = list(easy_set & neg_set)
            hard_neg_inds = list(hard_set & neg_set)

            num_expected_hard = int(
                num_expected * self.hard_fraction
            )
            if len(hard_neg_inds) > num_expected_hard:
                ...
                sampled_hard_inds = np.array(
                    hard_neg_inds, dtype=np.int
                )
            ...
            if len(easy_neg_inds) > num_expected_easy:
                ...
                sampled_easy_inds = np.array(
                    easy_neg_inds, dtype=np.int
                )
            ...
            if len(sampled_inds) < num_expected:
                ...
                extra_inds = np.array(
                    list(neg_set - set(sampled_inds))
                )
                if len(extra_inds) > num_extra:
                    extra_inds = self.random_choice(
                        extra_inds, num_extra
                    )
                sampled_inds = np.concatenate(
                    (sampled_inds, extra_inds)
                )
            sampled_inds = (
                torch.from_numpy(sampled_inds)
                .long()
                .to(assign_result.gt_inds.device)
            )
            return sampled_inds
