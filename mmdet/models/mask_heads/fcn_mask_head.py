...
class FCNMaskHead(nn.Module):
    def __init__(
        ...
        if upsample_method not in [
            ...
            raise ValueError(
                ...
                'are "deconv", "nearest", "bilinear"'.format(
                    upsample_method
                )
            )
        ...
        self.conv_logits = nn.Conv2d(
            self.conv_out_channels, out_channels, 1
        )
    ...
    def get_seg_masks(
        ...
        for i in range(bboxes.shape[0]):
            ...
            bbox_mask = (
                bbox_mask > rcnn_test_cfg.mask_thr_binary
            ).astype(np.uint8)
            im_mask[
                bbox[1] : bbox[1] + h, bbox[0] : bbox[0] + w
            ] = bbox_mask
