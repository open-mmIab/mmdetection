...
class AnchorHead(nn.Module):
    ...
    def __init__(
        ...
        for anchor_base in self.anchor_base_sizes:
            self.anchor_generators.append(
                AnchorGenerator(
                    anchor_base, anchor_scales, anchor_ratios
                )
            )

        self.num_anchors = len(self.anchor_ratios) * len(
            self.anchor_scales
        )
    ...
    def _init_layers(self):
        ...
        self.conv_reg = nn.Conv2d(
            self.feat_channels, self.num_anchors * 4, 1
        )
    ...
    def get_anchors(self, featmap_sizes, img_metas):
        ...
        for img_id, img_meta in enumerate(img_metas):
            ...
            for i in range(num_levels):
                ...
                valid_feat_h = min(
                    int(np.ceil(h / anchor_stride)), feat_h
                )
                valid_feat_w = min(
                    int(np.ceil(w / anchor_stride)), feat_w
                )
    ...
    def loss(
        ...
        label_channels = (
            self.cls_out_channels if self.use_sigmoid_cls else 1
        )
        ...
        num_total_samples = (
            num_total_pos
            if self.use_focal_loss
            else num_total_pos + num_total_neg
        )
    ...
    def get_bboxes_single(
        ...
        assert (
            len(cls_scores) == len(bbox_preds) == len(mlvl_anchors)
        )
        mlvl_bboxes = []
        mlvl_scores = []
