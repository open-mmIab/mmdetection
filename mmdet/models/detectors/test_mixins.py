...
class BBoxTestMixin(object):
    def simple_test_bboxes(
        self, x, img_meta, proposals, rcnn_test_cfg, rescale=False
    ):
...
class MaskTestMixin(object):
    def simple_test_mask(
        self, x, img_meta, det_bboxes, det_labels, rescale=False
    ):
        ...
        if det_bboxes.shape[0] == 0:
            ...
            for x, img_meta in zip(feats, img_metas):
                ...
                _bboxes = bbox_mapping(
                    det_bboxes[:, :4], img_shape, scale_factor, flip
                )
                mask_rois = bbox2roi([_bboxes])
                mask_feats = self.mask_roi_extractor(
                    x[
                        : len(
                            self.mask_roi_extractor.featmap_strides
                        )
                    ],
                    mask_rois,
                )
            ...
            merged_masks = merge_aug_masks(
                aug_masks, img_metas, self.test_cfg.rcnn
            )
