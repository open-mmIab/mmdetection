...
class CascadeRCNN(BaseDetector, RPNTestMixin):
    def __init__(
        ...
        if mask_head is not None:
            ...
            assert (
                len(mask_roi_extractor)
                == len(mask_head)
                == self.num_stages
            )
            for roi_extractor, head in zip(
                mask_roi_extractor, mask_head
            ):
    ...
    def with_rpn(self):
        return (
            hasattr(self, "rpn_head") and self.rpn_head is not None
        )
    ...
    def forward_train(
        ...
        for i in range(self.num_stages):
            ...
            if i < self.num_stages - 1:
                pos_is_gts = [
                    res.pos_is_gt for res in sampling_results
                ]
                roi_labels = bbox_targets[
                    0
    ...
    def simple_test(
        ...
        for i in range(self.num_stages):
            ...
            if self.test_cfg.keep_all_stages:
                ...
                bbox_result = bbox2result(
                    det_bboxes, det_labels, bbox_head.num_classes
                )
                ...
                if self.with_mask:
                    ...
                    if det_bboxes.shape[0] == 0:
                        ...
                        mask_feats = mask_roi_extractor(
                            x[
                                : len(
                                    mask_roi_extractor.featmap_strides
                                )
                            ],
                            mask_rois,
                        )
        ...
        if self.with_mask:
            if det_bboxes.shape[0] == 0:
                segm_result = [
                    []
                    for _ in range(
                        self.mask_head[-1].num_classes - 1
                    )
                ]
            else:
                _bboxes = (
        ...
        if not self.test_cfg.keep_all_stages:
            ...
            if self.with_mask:
                results = {
                    stage: (
                        ms_bbox_result[stage],
                        ms_segm_result[stage],
                    )
    ...
    def show_result(self, data, result, img_norm_cfg, **kwargs):
        ...
        super(CascadeRCNN, self).show_result(
            data, result, img_norm_cfg, **kwargs
        )
