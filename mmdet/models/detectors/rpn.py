...
class RPN(BaseDetector, RPNTestMixin):
    def __init__(
        ...
        self.neck = (
            builder.build_neck(neck) if neck is not None else None
        )
    ...
    def forward_train(
        self, img, img_meta, gt_bboxes=None, gt_bboxes_ignore=None
    ):
    ...
    def aug_test(self, imgs, img_metas, rescale=False):
        ...
        if not rescale:
            for proposals, img_meta in zip(
                proposal_list, img_metas[0]
            ):
    ...
    def show_result(
        self, data, result, img_norm_cfg, dataset=None, top_k=20
    ):
